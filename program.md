---
layout: default
title: Program
---

# SimpleText@CLEF-2026 Program

{% include navbar.md %}

------------------------------------------------------------

* CLEF 2026 registration
* CLEF 2026 program
* CLEF 2026 LNCS Proceedings
* CLEF 2026 CEUR Working Notes
  
------------------------------------------------------------

SimpleText Track sessions take place in **Unknown Room **, except for the plenary CLEF conference sessions in **Unknown Room**.


## Monday 21st September 

### Lab Overviews Session 1 (Lecture Room 3) 

* **11:40 – 12:30** **L. Ermakova**, H. Azarbonyad, J. Bakker, B. Chakar, G.K. Shahi, J. Kamps, _Overview of the CLEF 2026 SimpleText Track: Simplify Scientific Texts (and Nothing More)_ in: Proceedings of CLEF'26, LNCS, Springer, 2026 ([Paper](XXX), [Slides](XXX)). 

### SimpleText Track Session 1/3 (Room ???)

* **14:00-15:30** CLEF 2026 SimpleText Task Overviews:

14:00	–	15:30	SimpleText 1/3	

### SimpleText Track Session 2/3 (Room ???)

* **16:00 - 17:30** CLEF 2026 Participant presentations:

* To be announced.
  
## Tuesday 22nd September 

### SimpleText Track Session 3/3 (Room ???)

* **14:00	–	15:30** CLEF 2026 Participant presentations:

* To be announced.

## Wednesday 23rd September 

* To be announced.

## Thursday 24th September 

### Closing Ceremony and Introduction of CLEF 2027 

* **16:00	–	17:30** Closing and plans for 2027 (including slides on the CLEF 2027 SimpleText track, Slides).
------------------------------------------------------------

## All CLEF 2026 SimpleText track papers 

### Including authors unable to present in person or online:

SimpleText: Simplify Scientific Text (and Nothing More)

* Jan Bakker, Liana Ermakova, Jaap Kamps
_Overview of the CLEF 2026 SimpleText Task 1: Simplify Scientific Text_, 5907-5928
([Paper](https://clef-staging.pages.dev/paper444.pdf)).

* Berkay Chakar, Jan Bakker, Liana Ermakova, Jaap Kamps
_Overview of the CLEF 2026 SimpleText Task 2: Identify and Avoid Hallucination_, 5929-5944
([Paper](https://clef-staging.pages.dev/paper445.pdf)).

* Gautam Kishore Shahi, Liana Ermakova, Jaap Kamps
_Overview of the CLEF 2026 SimpleText Task 3: Research Area Classification_, 5945-5953
([Paper](https://clef-staging.pages.dev/paper446.pdf)).

* Mhd Adnan, Riad Sonbol
_A_R Team at CLEF2026 SimpleText Task 2.1: Source-Aware Grounding Verification with Scientific Transformer Ensembles_, 5954-5960
([Paper](https://clef-staging.pages.dev/paper447.pdf)).

* George Anderson, Edwin Thuma
_Improving Scientific Research Area Classification through Transformer Fine-Tuning and Per-Document Model Selection_, 5961-5973
([Paper](https://clef-staging.pages.dev/paper448.pdf)).

* Pablo Anegón Núñez, Paloma Martínez, Lourdes Moreno
_HULAT-UC3M at CLEF 2026 SimpleText: Controlled Generation for Scientific Text Simplification with Hallucination Control_, 5974-5984
([Paper](https://clef-staging.pages.dev/paper449.pdf)).

* Georgios Arampatzis, Avi Arampatzis
_DUTH at CLEF 2026 SimpleText: Grounded Biomedical Text Simplification and Evidence-Guided Overgeneration Detection_, 5985-6009
([Paper](https://clef-staging.pages.dev/paper450.pdf)).

* Artemis Capari, Hosein Azarbonyad, Zubair Afzal, George Tsatsaronis
_Elsevier at CLEF 2026 SimpleText: Deconstructing Scientific Text Simplification with Direct and Multi-Stage LLM Approaches_, 6010-6034
([Paper](https://clef-staging.pages.dev/paper451.pdf)).

* Feifan Chen, Leilei Kong, Yong Lu
_MONKEY at SimpleText 2026: Static Few-Shot Prompting for Cochrane Review Scientific Text Simplification_, 6035-6044
([Paper](https://clef-staging.pages.dev/paper452.pdf)).

* Run Chen, Yang Yang, Huaiyu Zhang, Leilei Kong
_FOSU_cr at the CLEF 2026 SimpleText Track: A Reference-Guided Few-Shot Prompting Approach for Biomedical Text Simplification_, 6045-6055
([Paper](https://clef-staging.pages.dev/paper453.pdf)).

* Sandra Conde, Pablo Folgueira, Alberto Díaz
_NIL-UCM at the CLEF 2026 SimpleText Track: Prompting, Fine-Tuning, and Multi-Agent Architectures for Biomedical Text Simplification_, 6056-6067
([Paper](https://clef-staging.pages.dev/paper454.pdf)).

* David Condrey
_Writerslogic at the CLEF 2026 SimpleText Track: Multi-Candidate LLM Simplification and Stacked Complexity Spotting_, 6068-6078
([Paper](https://clef-staging.pages.dev/paper455.pdf)).

* Julian Dauenhauer, Nico Hofmann, Christin Katharina Kreutz
_THM@SimpleText 2026 - Task 2: Ensemble-based Hallucination Classification in Text Simplification_, 6079-6087
([Paper](https://clef-staging.pages.dev/paper456.pdf)).

* Ella Hawkins, Kristina Zbinden, Ellis Fitzgerald, Behrooz Mansouri
_AIIRLab Systems at SimpleText CLEF 2026: Ensemble and Multi-Agent Pipelines for Scientific Text Simplification_, 6088-6099
([Paper](https://clef-staging.pages.dev/paper457.pdf)).

* Akio Hayakawa, Horacio Saggion
_UPF-TALN at the CLEF 2026 SimpleText Track: Ensemble Approach for Scientific Text Simplification and Hallucination Classification_, 6100-6114
([Paper](https://clef-staging.pages.dev/paper458.pdf)).

* Marc Hurtado, Vicent Ahuir, Antonio Molina, María-José Castro-Bleda
_ELiRF-UPV at SimpleText 2026: Decoupling Action Prediction and Controlled Generation for Text Simplification_, 6115-6132
([Paper](https://clef-staging.pages.dev/paper459.pdf)).

* Shimaa Ibrahim, Wajdi Zaghouani
_NUQLab at SimpleText 2026: Plan-Guided Biomedical Scientific Text Simplification_, 6133-6139
([Paper](https://clef-staging.pages.dev/paper460.pdf)).

* Manoj Kumar, Prabavathy Balasundaram, Deeraj Kumar
_Multilingual Scientific Text Simplification Using a Locally Deployed Open-Source LLM: Plan-Driven Sentence Simplification and Summary-Guided Document Simplification with Llama 3.1 8B at the CLEF 2026 SimpleText Track --- Task 1_, 6140-6152
([Paper](https://clef-staging.pages.dev/paper461.pdf)).

* Wenlong Liu, Sixian Qi, Yusheng Yi, Ruilan Lin
_FutureMakers at the CLEF 2026 SimpleText Track: Retrieval-Augmented Evidence-Focused Overgeneration Detection for Task 2.2_, 6153-6159
([Paper](https://clef-staging.pages.dev/paper462.pdf)).

* Sujith M, Sree Krishna S, Varghese K James, Prabavathy Balasundaram
_SSN Tokatrons at the CLEF 2026 SimpleText Track: Plan-Guided BART and Zero-Shot LLM Approaches to Biomedical Text Simplification_, 6160-6169
([Paper](https://clef-staging.pages.dev/paper463.pdf)).

* Pascal Mathas, Berkay Chakar, David Carranza Navarrete, Jan Bakker, Jaap Kamps
_University of Amsterdam at the CLEF 2026 SimpleText Track_, 6170-6179
([Paper](https://clef-staging.pages.dev/paper464.pdf)).

* Sejal Maurya, Ridhi Ladda, Gayatri Tupe, Ritesh Kumar
_BioSimplex at the CLEF 2026 SimpleText Task 1.1: A Two-Stage Operation-Aware Framework for Scientific Text Simplification_, 6180-6189
([Paper](https://clef-staging.pages.dev/paper465.pdf)).

* Sujaudeen Nannai John, Srinidhi Rajagopal, Yadushree Venkatesh
_DataDumplings at SimpleText 2026: Hierarchical Scientific Domain Classification with Fine-Tuned SciBERT and Taxonomy-Guided Inference_, 6190-6195
([Paper](https://clef-staging.pages.dev/paper466.pdf)).

* Clyde Lanvin Njinpie Noutche, Christin Katharina Kreutz
_clnn88@SimpleText 2026 - Task 1: Llama 3-based Simplification for Biomedical Text_, 6196-6202
([Paper](https://clef-staging.pages.dev/paper467.pdf)).

* Nuria Pirvu, Rémi Cardon
_HULAT-HAI @ SimpleText 2026 Task 1.1: Prompting Based on Linguistic Analysis for Text Simplification_, 6203-6211
([Paper](https://clef-staging.pages.dev/paper468.pdf)).

* Pratik Priyanshu, Swati Chandna
_SRH-ReliableAI at SimpleText 2026 Task 3: A Hierarchy-Constrained, Adversarially Trained Multi-Task Ensemble for Scientific Research Area Classification_, 6212-6221
([Paper](https://clef-staging.pages.dev/paper469.pdf)).

* Raneem Rabih, Riad Sonbol, Wesam Alsohle
_SPU_DeepSimplify at the CLEF 2026 SimpleText Track: A Planning-Based Approach to Scientific Text Simplification_, 6222-6232
([Paper](https://clef-staging.pages.dev/paper470.pdf)).

* Benjamin Vendeville, Liana Ermakova, Pierre De Loor
_UBONLP Report on the SimpleText Track at CLEF 2026_, 6233-6243
([Paper](https://clef-staging.pages.dev/paper471.pdf)).

* Shih-Hung Wu, Chong-Zhe Yan
_CYUT at CLEF 2026 SimpleText Track: Hallucination Detection based on Retrieval-Aligned Contextual Fusion and Multi-Tier Cascaded Discernment_, 6244-6254
([Paper](https://clef-staging.pages.dev/paper472.pdf)).

* Zonghua Yang, Haoliang Qi, Yusheng Yi
_FOSU-YZH-Group at the SimpleText 2026 Track: Statistics-Aware Prompting for Task 1.1 and Exploratory Overgeneration Detection for Task 2_, 6255-6266
([Paper](https://clef-staging.pages.dev/paper473.pdf)).
